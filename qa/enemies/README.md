...
Performance identities are inspected together immediately after one actual render. Optional hero LOD arrival can rebuild the scene between separate automation messages; the next game render repopulates the actors before drawing. Reading that intermediate preparation state caused a CI false negative even though two preceding enemy LODs were correct. The atomic snapshot retains all 30 identity, geometry, anatomy and material assertions, and records the frame's actor count and WebGL error. The independent missing-asset cold start closes the completed main scene first, so two continuously rendering software-GPU contexts do not compete for startup resources. Startup still has explicit readiness assertions and bounded deadlines.
