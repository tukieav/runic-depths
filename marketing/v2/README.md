# Runic Depths 2.1 submission media

Use these files for the current **The Hollow Covenant** game with the 2.1
graphics upgrade; the parent marketing directory contains historical v1 material.
...
The previews are silent H.264 MP4 files. Each starts with its matching static cover for 0.5 seconds, followed by normal-speed gameplay. The source footage has no mouse cursor, letterboxing, loading screen, or debug overlay. `manifest.json` records the encoded dimensions, duration, file sizes, audio-track checks, asset provenance, encounter staging, decoded asset status, and SHA-256 hashes of the captured build and asset manifests.

The footage shows the new animated GLB characters, original PBR surfaces and
sculpted environment props. Capture defaults to High graphics, including dynamic
shadows and point lights. Software Chromium capture uses an internal pixel ratio
of 0.8; the output and interface retain the listed full resolutions. This is an
actual game capture, not an offline renderer or a performance benchmark on a
...
Requires Playwright, Chromium (`CHROME_PATH` can override `/usr/bin/google-chrome`), FFmpeg, and ffprobe. The script serves the current `dist/` locally on a random loopback port. `RUNIC_GAME_URL` can select another local build with `?qa=1`. `RUNIC_CAPTURE_QUALITY=low` selects Performance graphics for a constrained capture machine and records that choice in the manifest. `--covers-only` renders only the three covers.
